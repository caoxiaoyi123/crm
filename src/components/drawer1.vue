                ...
                <li class="mb10" v-for="(item, i) of recordList" :key="i">
                  <div class="tit-txt fs13">
                    <font
                      ...
                      >!</font
                    >
                    {{ item.createtime }} <b>[{{ item.status }}]</b>
                    {{ item.userName }}添加了反馈
                  </div>
              ...
              <el-form @submit.native.prevent>
                <el-form-item label="相关文档" label-width="75px" v-if="!from">
                  <el-upload
                    ...
                    :on-remove="removeFn"
                  >
                    <button class="fs12 text-c cp upload-btn ml30">
                      上传文件
                    </button>
                  </el-upload>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    previewFn(item) {
      if (item) {
        this.openLink(baseUrl + "/so/file/view?fileId=" + item.fileId);
      } else {
        this.openLink(baseUrl + "/so/file/view?fileId=" + this.file.id);
      }
    },
    removeFn() {
      this.file.id = null;
