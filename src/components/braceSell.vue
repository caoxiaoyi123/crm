            ...
            >
                ...
                <el-table-column align="center"  min-width="140"  label="地区" prop="cityDetail">
                    <template slot-scope="scope">
                        <el-tooltip :content="returnRegion(scope.row.cityDetail)" placement="right">
                            <span class="text-over">
                                {{ returnRegion(scope.row.cityDetail) }}
                            </span>
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column align="left" header-align="center"  min-width="150" label="单位名称" prop="compName">
                    <template slot-scope="scope">
                ...
                <el-table-column align="left" header-align="center"  min-width="150" label="项目名称" prop="projName">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" label="预估金额（万）" width="120">
                    <template slot-scope="scope">
                        <el-input type="number" v-model="scope.row.gusAmount" placeholder="点击输入" @input="gusAmountBlur(scope.row,0)"/>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="概率" >
                    <template slot-scope="scope">
                        <el-input type="number" v-model="scope.row.probability" placeholder="点击输入" @input="probabilityFn(scope.row)"/>
                    </template>
                </el-table-column>
                        ...
                        <template v-if="scope.row.arr[o-1].id">
                            <el-tooltip placement="top" effect="light">
                                <div slot="content">
                                    ...
                                    <p class="fs14"  v-if="scope.row.arr[o-1].param=='签约'||scope.row.arr[o-1].param=='回款'">金额：{{scope.row.arr[o-1].amount}}</p>
        ...
        <el-dialog width="370px" :visible="isDialog" :show-close="false" :append-to-body="true">
            ...
            <div class="main">
                <el-form v-model="fromData" label-width="60px" size="medium">
                    ...
                    <el-form-item label="金额(万)" prop="amount" v-if="fromData.param=='签约'||fromData.param=='回款'">
                        <el-input placeholder="请输入金额(万)" v-model="fromData.amount" @input="gusAmountBlur(fromData,1)"></el-input>
                    </el-form-item>
                    <el-form-item label="备注" prop="remark" class="text-line" style="margin-bottom:12px">
                        <el-input
                            placeholder="请输入备注"
                            type="textarea"
                            resize="none"
